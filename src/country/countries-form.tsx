            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th data-field='countryCode'><button type='button' id='sortCountryCode' onClick={sort}>{resource.country_code}</button></th>
                ...
                {state.list && state.list.length > 0 && state.list.map((item, i) => {
                  return (
                    <tr key={i} onClick={e => edit(e, item.countryCode)}>
                      ...
                      <td><Link to={`${item.countryCode}`}>{item.countryCode}</Link></td>
            ...
            {state.list && state.list.length > 0 && state.list.map((item, i) => {
              return (
                <li key={i} className='col s12 m6 l3 xl4' onClick={e => edit(e, item.countryCode)}>
                  <section>
                    <div>
                      <h4><Link to={`${item.countryCode}`}>{item.countryCode} - {item.currencyCode}</Link></h4>
                      <p className='space-between'>{item.countryName} <span>{item.currencySymbol}</span></p>
                    </div>
