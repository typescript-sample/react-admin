...
export interface BaseEditComponentParam<T, ID> {
  ...
  load?: (
    i: ID | null,
    callback?: (m: T, showM: (m2: T) => void) => void
  ) => void;
  doSave?: (obj: T, diff?: T, version?: string, isBack?: boolean) => void;
  // prepareCustomData?: (data: any) => void; // need to review
}
export interface HookBaseEditParameter<T, ID, S>
  extends BaseEditComponentParam<T, ID> {
  refForm: any;
  initialState: S;
...
export interface EditComponentParam<T, ID, S>
  extends BaseEditComponentParam<T, ID> {
  initialize?: (
    id: ID | null,
...
export interface HookPropsEditParameter<T, ID, S, P>
  extends HookPropsBaseEditParameter<T, ID, S, P> {
  initialize?: (
    id: ID | null,
    ld: (i: ID | null, cb?: (m: T, showF: (model: T) => void) => void) => void,
    setState2: DispatchWithCallback<Partial<S>>,
...
export interface HookPropsBaseEditParameter<T, ID, S, P>
  extends HookBaseEditParameter<T, ID, S> {
  props: P;
...
export const useEdit = <T, ID, S>(
  ...
  useEffect(() => {
    ...
    return;
  }, [baseProps, p, p2.ui, params, refForm, service]);
  return { ...baseProps };
};
export const useEditProps = <T, ID, S, P>(
  ...
  const params = useParams();
  const baseProps = useCoreEdit<T, ID, S, P>(
    refForm,
    initialState,
    service,
    p2,
...
export const useEditOneProps = <T, ID, S, P>(
  p: HookPropsEditParameter<T, ID, S, P>
) => {
...
export const useCoreEdit = <T, ID, S, P>(
  ...
  const handleNotFound =
    p && p.handleNotFound ? p.handleNotFound : _handleNotFound;
    ...
    if (flag.newMode === true && flag.addable === false) {
      ...
      if (flag.newMode) {
        ...
        const diffObj = makeDiff(
          initPropertyNullInModel(flag.originalModel, metadata),
          obj,
          keys,
          version
        );
  ...
  const _validate = (obj: T, callback: (obj2?: T) => void) => {
    if (p1.ui) {
      const valid = p1.ui.validateForm(
        refForm.current,
        localeOf(undefined, p1.getLocale)
      );
      if (valid) {
        callback(obj);
      }
  ...
  const handleDuplicateKey =
    p && p.handleDuplicateKey ? p.handleDuplicateKey : _handleDuplicateKey;
  ...
  const _doSave = (obj: T, body?: T, version?: string, isBack?: boolean) => {
    ...
    if (flag.newMode === false) {
      if (
        ...
        service
          .patch(body)
          .then((result) => postSave(obj, result, version, isBackO));
      } else {
        service
          .update(obj)
          .then((result) => postSave(obj, result, version, isBackO));
